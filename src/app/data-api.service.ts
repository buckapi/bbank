...
export class DataApiService {	
  ...
  getTotalTransactions(userId: string){
  		let indice = userId;
  		const url_api =  "https://db.buckapi.us:9999/api/transaction?filter[where][userId]=p"+indice;
		return (this.transactions = this.http.get(url_api));
	}
	getInfo(){
		const url_api=`https://db.buckapi.us:9999/api/infos/`;
		this.info = this.http.get(url_api);
		return (this.info);
	}
	getNewCreditcardsReturn(){
		const url_api = 'https://db.buckapi.us:9999/api/creditcard?filter[where][status]=new';
		return (this.creditcards = this.http.get(url_api));
	}
	getActiveCreditcardsReturn(){
		const url_api = 'https://db.buckapi.us:9999/api/creditcard?filter[where][status]=complete';
		return (this.creditcards = this.http.get(url_api));
	}	
	getActiveAccountsReturn(){
		const url_api = 'https://db.buckapi.us:9999/api/account?filter[where][status]=active';
		return (this.accounts = this.http.get(url_api));
	}	
	getTransationsReturn(){
		const url_api = 'https://db.buckapi.us:9999/api/transaction';
		return (this.transactions = this.http.get(url_api));
	}	
	getNewAccountsReturn(){
		const url_api = 'https://db.buckapi.us:9999/api/account?filter[where][status]=new';
		return (this.accounts = this.http.get(url_api));
	}	
	...
	getAccountByUserd2(userId: string){
		let indice = userId;
		const url_api =  "https://db.buckapi.us:9999/api/account?filter[where][userId]=p"+indice;
		this.account = this.http.get(url_api);
		return (this.account);
	}
	getAccountByUserID(userId: string){
		let indice = userId;
		const url_api =  "https://db.buckapi.us:9999/api/account?filter[where][userId]="+indice;
		this.account = this.http.get(url_api);
		return (this.account);
	}
	settingsUpdate(info :InfoInterface, id: string){
		// let token = this.authService.getToken();
		const url_api=`https://db.buckapi.us:9999/api/infos/${id}`;
		return this.http
	...
	getCreditcardsByUserId(userId: string){
		let indice = userId;
		const url_api =  "https://db.buckapi.us:9999/api/creditcard?filter[where][userId]=p"+indice;
		this.creditcard = this.http.get(url_api);
		return (this.creditcard);
	}
	getAccountByEmail(userEmail: string){
		let email = userEmail;
		const url_api =  "https://db.buckapi.us:9999/api/account?filter[where][email]="+email;
		this.account = this.http.get(url_api);
		return (this.account);
	}
}